...
(function($) {
  $.widget("ra.filteringMultiselect", {
    ...
    _bindEvents: function() {
      ...
      /* HACK: update real select options*/
      widget.selection.change(function() {
        widget.selection.find("option").each(function(i, option) {
          var $realSelect = widget.selection.parents(".enum_type").find("select[data-options]");
          $realSelect.find("option[value='" + $(option).val() + "']").appendTo($realSelect);
        });
      });
    ...
    _buildCache: function(options) {
      ...
      $(this.options.values).each(function(i, value) {
        widget.selection.find("option[value='" + value + "']").appendTo(widget.selection);
      });
    },
