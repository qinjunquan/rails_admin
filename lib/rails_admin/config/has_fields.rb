module RailsAdmin
  module Config
    ...
    module HasFields
      ...
      def field(name, type = nil, inline_block = nil, add_to_section = true, &block)
        ...
        field.instance_eval(&inline_block) if inline_block.is_a?(Proc)
        field
      end
      ...
      def fields(*field_names, &block)
        ...
        if field_names.empty?
          ...
          defined = field_names.collect do |field_name|
            f = _fields.detect { |f| f.name == field_name }
            if f.nil?
              f = RailsAdmin::Config::Fields::Types::String.new(parent, field_name, {})
              _fields << f
            end
            f
          end
        end
        my_fields = defined.collect do |f|
          unless f.defined
            f.defined = true
            f.order = _fields.select(&:defined).length
          end
        ...
        exclude_fields(*(_fields.map(&:name) - my_fields.map(&:name) - [:id, :created_at, :updated_at]))
        _fields.each { |f| f.order = field_names.index(f.name).to_i }
        my_fields
      end
